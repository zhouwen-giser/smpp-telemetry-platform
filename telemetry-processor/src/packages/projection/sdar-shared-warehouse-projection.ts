...
export class SdarSharedWarehouseProjectionV1{
  ...
  project(fact){
    ...
    const provider={
      ...
      origin_sdar_runtime_ids:[...(fact.correlation?.originRuntimeInstanceIds??[])],origin_sdar_task_ids:[...(fact.correlation?.originTaskIds??[])],origin_sdar_invocation_ids:[...(fact.correlation?.originInvocationIds??[])],
      ...
      occurred_at:fact.occurredAt,observed_at:semantics.observedAt??null,received_at:fact.receivedAt,normalized_at:fact.normalizedAt,
