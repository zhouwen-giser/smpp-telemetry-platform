...
test('shadow parity fails closed for hash, lag and relation gaps',()=>{
  const result=compareSdarShadowParity({standaloneFacts:facts,sdarFacts:[{...at(facts,0),factHash:'c'.repeat(64),projectedAt:'2026-08-18T00:59:00.000Z'}],relations:[],expectedRelationFactIds:['fact-a','fact-b'],maxWatermarkLagMs:1000,minimumRelationCoverage:1});
...
test('same identity with different hash is an explicit content conflict',()=>{
  assert.throws(()=>compareSdarShadowParity({standaloneFacts:[at(facts,0),{...at(facts,0),factHash:'c'.repeat(64)}],sdarFacts:[],relations:[],expectedRelationFactIds:[],maxWatermarkLagMs:1000,minimumRelationCoverage:1}),/SMPP_FACT_CONTENT_CONFLICT/);
});
