...
test('projection rows carry processor time across targets with opposite server clock drift',async()=>{
  ...
  class DriftedTarget extends Fake{
    constructor(readonly serverNow:string){super();}
    async insert(table:string,rows:readonly ProjectionRow[]){this.rows.push([table,rows.map(row=>({...row,projected_at:row.projected_at??this.serverNow}))]);}
  }
  const clients={
  ...
  const manager=new TargetManager({targets,wal,metrics:new Metrics(),clock:{now:()=>processorTime},clientFactory:target=>clientFor(clients,target.targetId)});
  at(manager.targets,1).schemaPreflight={assert:async()=>true};
  ...
  assert.deepEqual(manager.statuses().map(status=>[status.checkpoint.segment,status.checkpoint.offsetEnd]),[[1,at(manager.statuses(),0).checkpoint.offsetEnd],[1,at(manager.statuses(),0).checkpoint.offsetEnd]]);
});
...
test('projection fails closed when processor projection time precedes receiver time',async()=>{
  ...
  await manager.initialize();
  await manager.flush();
  assert.equal(at(manager.statuses(),0).lastError,'PROCESSOR_PROJECTION_CLOCK_BEFORE_RECEIVE');
  assert.equal(at(manager.statuses(),0).pending,1);
