interface FactRow { fact_id:string; fact_hash:string; fact_type:string; projected_at:string; source_schema_version?:string; projection_id?:string; projection_version?:number }
interface RelationRow { relation_id:string; evidence_fact_ids:string[]; projection_id?:string; projection_version?:number }
...
const sharedUrl=process.env.CLICKHOUSE_URL;if(!sharedUrl)throw new Error('CLICKHOUSE_URL_REQUIRED');
const sdar=new ClickHouseClient({url:sharedUrl,user:process.env.CLICKHOUSE_USER??'default',password:process.env.CLICKHOUSE_PASSWORD??''});
...
const standaloneFacts=await rows<FactRow>(standalone,`SELECT toString(fact_id) AS fact_id,fact_hash,fact_type,toString(normalized_at) AS projected_at FROM telemetry_normalized.canonical_fact_v1 FINAL WHERE ${scope} ORDER BY fact_id FORMAT JSON`);
const sdarFacts=await rows<FactRow>(sdar,`SELECT toString(fact_id) AS fact_id,fact_hash,fact_type,toString(projected_at) AS projected_at,source_schema_version,projection_id,projection_version FROM sdar_core.external_provider_fact FINAL WHERE ${sourceScope} ORDER BY fact_id FORMAT JSON`);
const standaloneRelations=await rows<RelationRow>(standalone,`SELECT toString(relation_id) AS relation_id,arrayMap(value -> toString(value),evidence_fact_ids) AS evidence_fact_ids FROM telemetry_core.entity_relation_fact FINAL WHERE ${scope} ORDER BY relation_id FORMAT JSON`);
const sdarRelations=await rows<RelationRow>(sdar,`SELECT toString(relation_id) AS relation_id,arrayMap(value -> toString(value),evidence_fact_ids) AS evidence_fact_ids,projection_id,projection_version FROM sdar_core.external_entity_relation_fact FINAL WHERE ${sourceScope} ORDER BY relation_id FORMAT JSON`);
...
async function rows<Row>(client: ClickHouseClient,sql: string): Promise<Row[]>{const result=JSON.parse(await client.query(sql));if(!Array.isArray(result.data))throw new Error('SMPP_E2E_QUERY_INVALID');return result.data;}
