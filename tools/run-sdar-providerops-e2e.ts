...
const fixtures: {valid: {recordType: string; payload: Record<string, unknown>}[]}=JSON.parse(await readFile('contracts/provider-ops-payload-fixtures.v1.1.json','utf8'));
const category=(recordType: string)=>recordType.startsWith('provider.business_event.')?'business_event.lifecycle':recordType.slice('provider.'.length);
const occurredBase=Date.parse('2026-08-18T02:32:07.000Z');
const envelopes=fixtures.valid.map((fixture,index)=>{
  const envelope: ProviderOpsEnvelope={
...
const firstEnvelope=envelopes[0]; if(!firstEnvelope)throw new Error('SMPP_E2E_FIXTURES_EMPTY');
const conflict={...firstEnvelope,payload:{currentState:'failed',terminalStatus:'failed',providerSubstate:'error',providerRevision:'8',observedAt:'2026-08-18T02:32:07.000Z'}};
...
async function send(values: ProviderOpsEnvelope[]){
