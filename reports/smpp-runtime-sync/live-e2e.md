...
The separately governed physical-navigation flow produced a valid negative case. SDAR attempted dispatch with the frozen `simulation` mode, but the Runtime returned `UGV_EXECUTION_MODE_UNSUPPORTED`; no remote task, movement, DeviceMission, or authoritative relation was created. Before a governed real-mode retry, `SDAR-session` received a newer task-package request that explicitly replaced the unfinished navigation. This report therefore makes no physical-navigation success claim and confirms that Telemetry did not synthesize an identity for the rejected attempt.
